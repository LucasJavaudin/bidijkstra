...
def dijkstra(G, source, target, weight='weight'):
    ...
    results = [dict(), dict()]
    cdef FibonacciHeap fringe0 = FibonacciHeap()
    cdef FibonacciHeap fringe1 = FibonacciHeap()
    ...
    seen0[source] = 0
    seen1[target] = 0
    fringe0.insert(0, source)
    fringe1.insert(0, target)
    ...
    while not fringe0.empty() and not fringe1.empty():
        ...
        if d:
            p = fringe1.extractMin()
        else:
            p = fringe0.extractMin()
        ...
        for i in range(n):
            ...
            if (
                ...
                if d:
                    ...
                    fringe1.push(alt_dist, out_node)
                else:
                    seen0[out_node] = alt_dist
                    fringe0.push(alt_dist, out_node)
